...
def _load_or_train_model(cutoff_date: date = DEFAULT_CUTOFF):
    ...
    Returns (model, data_by_group, signature)
    """
    ...
    cache_key = f"model_{signature}_{cutoff_str}"
    cache_path = os.path.join(CACHE_DIR, f"{cache_key}.pkl")
    
    if os.path.exists(cache_path):
        with open(cache_path, "rb") as f:
            payload = pickle.load(f)
        return payload["model"], payload["data_by_group"], signature
    ...
    with open(cache_path, "wb") as f:
        pickle.dump({"model": model, "data_by_group": data_by_group}, f)
    return model, data_by_group, signature
...
def predict(req: PredictRequest):
    ...
    try:
        ...
        model, data_by_group, signature = _load_or_train_model(current_cutoff)
        ...
        pred_date = predict_next_release_lightgbm(
            df_group=df_group,
            model=model,
            group=group,
